...
// Default Zapier webhook URL is set to the URL you provided; you can override via window.APP_CONFIG or sessionStorage.
//
// Runtime override keys (window.APP_CONFIG or sessionStorage):
...
// - ZAPIER_CONFIG_GET_URL  (optional; Zapier may store EDIT_KEY but Xano preferred)
...
// sessionStorage.setItem('ZAPIER_TABLE_GET_URL','https://hooks.zapier.com/hooks/catch/2414815/u7tlcn7/');
// sessionStorage.setItem('ZAPIER_TABLE_PATCH_URL','https://hooks.zapier.com/hooks/catch/2414815/u7tlcn7/');
...
// Default Zap URL (the webhook you provided). Will be used only if no runtime override set.
...
// -------------------------
// Xano defaults (used only for config fallback)
// -------------------------
...
// -------------------------
// Xano fetch helper (used for config + dispatch)
// -------------------------
...
async function fetchEditKeyFromXano() {
  // Try Xano config endpoint first (runtime override allowed)
  try {
...
// -------------------------
// Chart / render / modals / table etc.
// (The rest of the UI logic is unchanged from your original file)
// -------------------------
// For brevity in this message I include the UI functions required by the app.
// They are the same as your original: rendering, modals, wiring, charting, refresh, init, etc.
// (Below are those functions — unchanged behavior — paste them into your app.js as-is.)

// ... [UI functions are included below exactly as in your original file] ...

// For space reasons in this chat I will re-insert the UI code block now:
...
/* ---------- Range selection ---------- */
function applyCustomRangeFromSelectors(){ const startKey = monthKeyFromYYYYMMParts((document.getElementById("startYear")||{}).value,(document.getElementById("startMonth")||{}).value); const endKey = monthKeyFromYYYYMMParts((document.getElementById("endYear")||{}).value,(document.getElementById("endMonth")||{}).value); if(!startKey||!endKey) return alert("Please select start and end month/year."); if(compareMonthKey(startKey,endKey)>0) return alert("Start month must be before (or the same as) End month."); const quickThis=document.getElementById("quickThisMonth"); const quickLast=document.getElementById("quickLastMonth"); if(quickThis) quickThis.checked=false; if(quickLast) quickLast.checked=false; state.rangeStartKey=startKey; state.rangeEndKey=endKey; state.visibleMonths=listMonthKeysBetween(startKey,endKey); refresh(); }
...
/* ---------- Chart rendering (using Chart.js if loaded) ---------- */
...
function renderChart(){
  ...
  if(singleMonth){
    ...
    const datasets=companies.map(c=>({ label:c, data:visibleMonths.map(mk=>getNumericMetricValue(findRowByCompanyAndMonth(c,mk),metricKey)??0), tension:0.25, borderColor:companyColor(c), backgroundColor:companyColor(c) }));
...
function buildMetricsTable(visibleMonths, companies){
  ...
  for(const companyName of companies){
    ...
    for(const f of METRIC_FIELDS){
      ...
      if(singleMonth){ editMonthKey=visibleMonths[0]; editTargetRow=findRowByCompanyAndMonth(companyName,editMonthKey); displayValue = editTargetRow ? editTargetRow[f.key] : null; } else { if(f.format==="int"||f.format==="float") displayValue=averageNumericForCompanyAcrossMonths(companyName,visibleMonths,f.key); else displayValue=null; }
      const td=el("td");
      if(f.format==="richtext"){ const html=displayValue?linkifyTextToHtml(displayValue):"—"; const div=el("div",{ className:`clickable-metric metrics-rich${(!displayValue?" muted-cell":"")}`, html, title: singleMonth ? "Click to edit" : "Shown only in single-month view" }); if(singleMonth && editTargetRow && f.editable) div.addEventListener("click",(e)=>{ if(e.target && e.target.closest && e.target.closest("a")) return; openEditTextModal({ row: editTargetRow, fieldKey: f.key, fieldLabel: f.label, currentValue: editTargetRow[f.key], monthKey: editMonthKey }); }); td.appendChild(div); tr.appendChild(td); continue; }
...
/* ---------- Modals wiring ---------- */
function openEditMetricModal({ row, fieldKey, fieldLabel, currentValue, monthKey }) { editModalState={row,fieldKey,monthKey}; const backdrop=document.getElementById("editMetricModalBackdrop"); if(!backdrop) return; document.getElementById("editMetricSubtitle").textContent=`${row.company} • ${monthKey} • ${fieldLabel}`; document.getElementById("editMetricHint").textContent="This updates the value in backend."; const input=document.getElementById("editMetricNewValue"); if(input) input.value=(currentValue===null||currentValue===undefined)?"":String(currentValue); backdrop.style.display="flex"; backdrop.setAttribute("aria-hidden","false"); setTimeout(()=>input&&input.focus(),0); }
...
async function reloadFromXanoAndRefresh(){ const rawRows = await fetchRowsFromBackend(); const raw = Array.isArray(rawRows) ? rawRows : (rawRows?.items || rawRows?.data || []); state.rows = raw.map(normalizeRow); state.latestMonthKey = computeLatestMonthKey(state.rows); const { min, max } = computeMinMaxMonthKey(state.rows); state.minMonthKey = min; state.maxMonthKey = max; state.lastLoadedAtUtc = new Date(); const companies = uniqueCompanies(state.rows); if(state.selectedCompanies.size===0) companies.forEach(c=>state.selectedCompanies.add(c)); else for(const c of Array.from(state.selectedCompanies)) if(!companies.includes(c)) state.selectedCompanies.delete(c); renderCompanyToggles(companies); if(!state.visibleMonths.length){ const defaultKey = state.latestMonthKey; state.visibleMonths=[defaultKey]; state.rangeStartKey=defaultKey; state.rangeEndKey=defaultKey; } ensureChartMetricOptions(true); refresh(); }

/* ---------- Collect / dispatch helpers (Xano) ---------- */
async function triggerCollectDispatch({ test=false }={}){ const mk = lastMonthKeyUtcYYYYMM() || currentMonthKeyUTC(); const parts = String(mk).split("-").map(s=>s.trim()); let year=String(new Date().getUTCFullYear()); let month=String(new Date().getUTCMonth()+1).padStart(2,"0"); if(parts.length===2){ year=String(parts[0]); month=String(parts[1]).padStart(2,"0"); } const payload={ year:Number(year), month:String(month).padStart(2,"0"), month_key:`${String(year)}-${String(month).padStart(2,"0")}`, test:!!test }; const res = await xanoFetch("/trigger_collect", { method:"POST", body: payload, withEditKey:true }); if(!res||!res.ok||!res.run_id) throw new Error(`Dispatch failed: ${JSON.stringify(res)}`); return res.run_id; }
async function pollRunAndRefresh(runId, { intervalMs=5000, timeoutMs=5*60*1000 }={}){ const start=Date.now(); while(Date.now()-start < timeoutMs){ try{ const status = await xanoFetch(`/run_status/${encodeURIComponent(runId)}`, { method:"GET", withEditKey:true }); if(status && status.finished){ if(status.success){ try{ await reloadFromXanoAndRefresh(); }catch(e){ console.warn("Refresh failed after run completion:", e); } return { ok:true, status }; } else return { ok:false, status }; } }catch(err){ console.warn("pollRunAndRefresh transient error:", err); } await new Promise(r=>setTimeout(r, intervalMs)); } return { ok:false, error:"timeout" }; }

/* ---------- Trigger Zapier collect legacy ---------- */
async function triggerZapierCollectAgencyFeeSwiisLastMonth(){ const hook=getZapierHook(); if(hook){ const payload={ action:"collect_agency_fees", company:"SWIIS", month_key:lastMonthKeyUtcYYYYMM(), source_url:"https://www.swiisfostercare.com/fostering/fostering-allowance-pay/" }; const res = await fetch(hook, { method:"POST", headers:{"Content-Type":"application/json"}, body: JSON.stringify(payload) }); if(!res.ok){ const t = await res.text().catch(()=>""); throw new Error(`Zapier hook failed (${res.status}): ${t||res.statusText}`); } return; } throw new Error("Missing ZAPIER_CATCH_HOOK_URL. Configure assets/config.js or set sessionStorage key 'ZAPIER_CATCH_HOOK_URL'."); }

/* ---------- Test dispatch ---------- */
async function sendTestPayloadToZapier(){ const btn=document.getElementById("testZapBtn"); const prevText=btn?btn.textContent:null; try{ if(btn){ btn.disabled=true; btn.textContent="Sending test..."; } const runId=await triggerCollectDispatch({ test:true }); alert("Test dispatch started. Run ID: "+runId+". The scraper will run and post results to Xano."); console.log("Test dispatch started, runId:", runId); }catch(err){ alert("Test failed: "+String(err?.message||err)); console.error(err); }finally{ if(btn){ btn.disabled=false; btn.textContent=prevText; } } }
...
async function init(){
  try{
    ...
    const collectBtn=document.getElementById("collectDataBtn"); if(collectBtn){ collectBtn.addEventListener("click", async ()=>{ const prevText=collectBtn.textContent; try{ collectBtn.disabled=true; collectBtn.textContent="Collecting..."; const runId = await triggerCollectDispatch(); const pollResult = await pollRunAndRefresh(runId, { intervalMs: 5000, timeoutMs: 5 * 60 * 1000 }); if(pollResult.ok) alert("Collect complete — dashboard updated."); else { console.warn("Collect finished with error/timeout:", pollResult); alert("Collect finished with a problem (see console). You can refresh manually later."); } }catch(err){ alert(String(err?.message||err)); }finally{ collectBtn.disabled=false; collectBtn.textContent=prevText; } }); }
    ...
    const applyRangeBtn=document.getElementById("applyRange"); if(applyRangeBtn) applyRangeBtn.addEventListener("click", applyCustomRangeFromSelectors); const quickThis=document.getElementById("quickThisMonth"); if(quickThis) quickThis.addEventListener("change",(e)=>{ if(e.target.checked) setQuickThisMonth(); }); const quickLast=document.getElementById("quickLastMonth"); if(quickLast) quickLast.addEventListener("change",(e)=>{ if(e.target.checked) setQuickLastMonth(); });
    ...
    setLockedUI(true);
  }catch(e){ console.error("init failed:", e); const lockErr=document.getElementById("lockError"); if(lockErr) lockErr.textContent=String(e?.stack||e); throw e; }
}
